...
VALIDATION_SET_NAME = 'output_train.tfrecords'
TEST_SET_NAME = 'test_set.tfrecords'
VALIDATION_SET_NAME_1 = 'output_cv.tfrecords'
ORIGIN_PREDICT_DIRECTORY = '../data_set/test'
PREDICT_SAVED_DIRECTORY = '../data_set/predictions'

INPUT_IMG_WIDE, INPUT_IMG_HEIGHT, INPUT_IMG_CHANNEL = 240, 240, 4
...
class Unet:
    ...
    def set_up_unet(self, batch_size):
        ...
        with tf.name_scope('input'):
            ...
            self.input_image = tf.placeholder(
                dtype=tf.float32,
                shape=[batch_size, INPUT_IMG_WIDE, INPUT_IMG_WIDE, INPUT_IMG_CHANNEL], name='input_images'
            )
            self.input_label = tf.placeholder(
                dtype=tf.int32,
                shape=[batch_size, OUTPUT_IMG_WIDE, OUTPUT_IMG_WIDE], name='input_labels'
            )
        ...
        with tf.name_scope('layer_1'):
            ...
            result_relu_2 = tf.nn.relu(normed_batch, name='relu_1')
        ...
        with tf.name_scope('softmax_loss'):
            ...
            """
            if self.is_traing:
                b_size = TRAIN_BATCH_SIZE
            else:
                b_size = 1
            self.loss_weight = tf.reshape(tf.Variable(initial_value=[0.002, 1, 1, 1], dtype=tf.float32, name='lossweight'), [1, 1, 1, 4])
            pre_softmax = tf.nn.softmax(self.prediction, axis=-1)
            label_vector = tf.reshape(self.input_label, [b_size*INPUT_IMG_HEIGHT*INPUT_IMG_WIDE], name='labelvector')
            with tf.device('/cpu:0'):
                label_vector_onehot = tf.one_hot(label_vector, depth=CLASS_NUM, axis=-1)

            label_onehot = tf.reshape(label_vector_onehot, [b_size, INPUT_IMG_HEIGHT, INPUT_IMG_WIDE, CLASS_NUM], name='labelonehot')
    ...
    def train(self):
        ...
        train_images, train_labels = read_img(train_image_filename_queue, TRAIN_BATCH_SIZE)
                    ...
                    for i in range(slices):
                        ...
                        los = sess.run(self.loss,
                                              feed_dict={self.input_image: example,
                                                         ...
                                                         self.is_traing: True})
"""
                        lo, acc, summary_str, losss = sess.run(
    ...
    def validate(self):
        ...
        ckpt_path = os.path.join(FLAGS.model_dir + "/model.ckpt")
        validation_images, validation_labels = read_img(validation_image_filename_queue, VALIDATION_BATCH_SIZE)
        # tf.summary.scalar("loss", self.loss_mean)
        # tf.summary.scalar('accuracy', self.accuracy)
        # merged_summary = tf.summary.merge_all()
        all_parameters_saver = tf.train.Saver()
            ...
            try:
                ...
                while not coord.should_stop():
                    ...
                    for i in range(slices):
                        ...
                        label = label_batch[:, :, :, i]
                        acc, pre = sess.run(
                            [self.accuracy, self.final_prediction],
                            feed_dict={
                                self.input_image: example, self.input_label: label, self.keep_prob: 0.5,
                                self.lamb: 0.5, self.is_traing: False}
                        )
                        h += 1
                    # summary_writer.add_summary(summary_str, epoch)
                    # print('num %d, loss: %.6f and accuracy: %.6f' % (epoch, lo, acc))
    ...
    def test(self):
        ...
        test_images, test_label`s = read_image_batch(test_image_filename_queue, TEST_BATCH_SIZE)
        # tf.summary.scalar("loss", self.loss_mean)
        # tf.summary.scalar('accuracy', self.accuracy)
        # merged_summary = tf.summary.merge_all()
        all_parameters_saver = tf.train.Saver()
